...
pub fn getCpuVendor() [12]u8 {
    ...
    std.mem.copy(u8, vendor[0..4], std.mem.asBytes(&result.ebx));
    std.mem.copy(u8, vendor[4..8], std.mem.asBytes(&result.edx));
    std.mem.copy(u8, vendor[8..12], std.mem.asBytes(&result.ecx));
    return vendor;
}
